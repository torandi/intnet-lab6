...
public abstract class DatabaseObject<T extends DatabaseObject<T>> {
	...
	public DatabaseObject() {
		...
		try {
			...
			while (rs.next()) {
				Field f = new Field(rs.getString("COLUMN_NAME"),
						rs.getInt("DATA_TYPE"));
				columns.add(f);
			}
	...
	public boolean delete() throws SQLException {
		...
		PreparedStatement stmt = DatabaseConnection.get().prepareStatement(
				"delete from `" + table_name() + "` where `" + id_name()
						+ "`=?");
		stmt.setInt(1, id);
		return stmt.execute();
	}
	...
	protected void set_from_db(ResultSetMetaData meta, ResultSet rs)
			throws SQLException {
	...
	protected void validateExistance(String attr) throws ValidationException {
		if (get(attr) == null)
			throw new ValidationException(attr, "Must be set.");
	}

	protected void validateMinLength(String attr, int min_length)
			throws ValidationException {
		validateExistance(attr);
		if (((String) get(attr)).length() < min_length)
			throw new ValidationException(attr, "Must be at least "
	...
	protected void validateUniqueness(String attr) throws ValidationException {
		try {
			...
			e.printStackTrace();
			throw new ValidationException(attr,
					"An SQL error occurred while trying to validate uniqueness");
		}
	}
